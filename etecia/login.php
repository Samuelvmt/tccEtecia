    ...
    <main class="lMain">
        ...
        <form action="php/validacao.php" method="post" class="lForm" style="background-color: rgba(0, 0, 0, 0.8);">
            ...
            <div class="container">
                ...
                <input type="text" placeholder="RM" class="lInput" name="login" id="login" required value="<?php echo isset($_COOKIE['login']) ? htmlspecialchars($_COOKIE['login']) : ''; ?>">
                <br>
                <label for="senha" class="lLabel"><b>Senha:</b></label>
                <input type="password" placeholder="Senha" class="lInput" name="senha" id="senha" required value="<?php echo isset($_COOKIE['senha']) ? htmlspecialchars($_COOKIE['senha']) : ''; ?>">
                <br>
                <button type="submit" class="lButton" value="entrar" id="entrar" name="entrar">Entrar</button>
                <br>
                <label class="checkboxLabel">
                    <input type="checkbox" name="remember" class="lInput" <?php echo isset($_COOKIE['login']) ? 'checked' : ''; ?>> Manter-me conectado
                </label>
            </div>
            <div class="container">
